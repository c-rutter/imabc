#' Approximate Bayesian Calibration Algorithm
#'
#' @description ABCLoopFunction creates 7 output files, results.dir is the results directory:
...
#' @param sample.inflate PLACEHOLDER
#' @param inputs.dir directory containing model inputs
#' @param prior.file contains prior distribution information for model parameters
#' @param default.file PLACEHOLDER
#' @param target.sims specification for simulations for each target
#' @param target.file PLACEHOLDER
#' @param model.parm.order PLACEHOLDER
#' @param userfn.dir dirctory containing user-specified functions
#' @param outfile.modelparms PLACEHOLDER
#' @param outfile.dist PLACEHOLDER
#' @param outfile.simparms PLACEHOLDER
#' @param outfile.sampling PLACEHOLDER
#' @param output.directory PLACEHOLDER
#' @param LHS PLACEHOLDER
#' @param continue.runs PLACEHOLDER
#' @param reeval reevaluate currently accepted runs (logical)
#' @param prevruns.dir PLACEHOLDER
#' @param print.status PLACEHOLDER
#' @param recalc.centers PLACEHOLDER
#'
#' @note While biased targets can be included, and are used for the Corley targets, the current implementation assumes
#' that estimtaes are downwardly biased. Code would need ot be generalized to allow upward bias.
#'
#' We incorporate one tpe of user-specified prior distribution: a resticted uniform, which is used to accomodate the
#' restricted range of beta2 growth parameters, which depend on beta1
#'
#' There is currently some specialized coding to allow continuation of prior runs. Continuation requres the use of the
#' same targets. Some code that can be removed correcs an issue with naming of SEER targets
#'
#' @return ABCLoopFunction creates 7 output files, results.dir is the results directory:
#'              1. outfile.modelparms: all drawn model parameters
#'              2. outfile.dist: distance from targets
#'              3. outfile.simparms: simulated targets at model parameters
#'              4. 'good.'outfile.modelparms: accepted model parameters
