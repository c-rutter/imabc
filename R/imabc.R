...
#' @param n_cons_fail_update numeric(1). Number of iterations that imabc will stop after no updates of the target bounds.
#'
#' @details
...
imabc <- function(
  ...
  for (main_loop_iter in start_iter:end_iter) {
    ...
    if (main_loop_iter < end_iter || main_loop_iter == 1) {
      ...
      if (max_fail_iter >= n_cons_fail_update) {
        warning("Unable to improve bounds after n_cons_fail_update tries. Stopping imabc early.")
        break
      }
