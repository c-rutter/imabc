...
#' @param calib_targets_wt numeric(1). The weight to apply to the total distance of calibrated targets or target groups.
...
#' @param improve_method character(1). Either "percentile", "direct" or "both".
#' @param max_fail_iter numeric(1). Number of iterations that imabc will stop after no updates of the target bounds.
#'
#' @details
...
imabc <- function(
  ...
  for (main_loop_iter in start_iter:end_iter) {
    ...
    done_targets <- unique(attr(targets, "target_groups"))[!unique(attr(targets, "target_groups")) %in% unique(attr(targets, "update"))]
    ...
    if (!is_first_continue_iter) {
      ...
      if (length(update_targets) == 0) {
        ...
        iter_target_dist$euclid_dist <- euclid_distance(
          dt = iter_target_dist, update_targets = update_targets, target_names = target_distance_names,
          wt = calib_targets_wt)
        ...
        iter_target_dist$tot_dist <- total_distance(
          dt = iter_target_dist, target_names = target_distance_names, update_targets = update_targets,
          scale = FALSE, wt = calib_targets_wt
        )
        # Set total distance to 0 for any observation that meets the stopping bounds
        check <- get_in_range(compare_list = targets, check_dt = iter_sim_target, criteria = "stop", out = "logical")
        if (priority_sets) {
        ...
        if (length(keep_draws) > 0) {
          ...
          if (length(update_targets) == 0) {
            ...
            good_target_dist[draw %in% keep_draws, euclid_dist := euclid_distance(
              ...
              update_targets = update_targets, target_names = target_distance_names,
              wt = calib_targets_wt
            )]
            # Rerun target distances but exclude targets that have been their stopping bounds and whose values are actually
            #   in the stopping bounds as well
            ...
            good_target_dist[draw %in% keep_draws, tot_dist := total_distance(
              dt = good_target_dist[draw %in% keep_draws], target_names = target_distance_names,
              update_targets = update_targets, scale = FALSE, wt = calib_targets_wt
            )]
            # Set total distance to 0 for any observation that meets the stopping bounds
            check <- get_in_range(compare_list = targets, check_dt = good_sim_target, criteria = "stop", out = "logical")
            if (priority_sets) {
      ...
      if (iter_valid_n > 0) {
        ...
        if (length(update_targets) == 0) {
          ...
          good_target_dist[update_row_range, tot_dist := total_distance(
            dt = good_target_dist[update_row_range], target_names = target_distance_names,
            update_targets = update_targets, scale = FALSE, wt = calib_targets_wt
          )]
          # Set total distance to 0 for any observation that meets the stopping bounds
          check <- get_in_range(compare_list = targets, check_dt = good_sim_target, criteria = "stop", out = "logical")
    ...
    if ((current_good_n >= improve_min_samplefactor*N_cov_points) & length(update_targets) > 0 & iter_valid_n > 0) {
      ...
      if (improve_method %in% c("percentile", "both")) {
        ...
        if (length(update_targets) == 0) {
          ...
          good_target_dist[, tot_dist := total_distance(
            dt = good_target_dist, target_names = target_distance_names,
            update_targets = update_targets, scale = FALSE, wt = calib_targets_wt
          )]
        }
        # Set total distance to 0 for any observation that meets the stopping bounds
      ...
      if (new_iter_valid_n >= N_cov_points) {
        ...
        updated_bounds <- improve # For next print
        n_cons_fail_update <- ifelse(improve, 0, n_cons_fail_update + 1)
      } else { # new_iter_valid_n >= N_cov_points & new_iter_valid_n < current_good_n
        updated_bounds <- FALSE # For next print
        n_cons_fail_update <- n_cons_fail_update + 1
      }
    ...
    } else { # (current_good_n >= improve_min_samplefactor*N_cov_points) & length(update_targets) > 0 & iter_valid_n > 0
      n_cons_fail_update <- n_cons_fail_update + 1
    }
    ...
    if (main_loop_iter < end_iter || main_loop_iter == 1) {
      ...
      if (current_good_n < N_cov_points) {
        ...
        for (center_sim_iter in 1:num_centers) {
          ...
          min_sample_var_pct <- getOption("imabc.min_sample_var_pct")
          ...
          if (any(is_small)) {
            ...
            message(paste0("Potentially over-concentrated sampling: Parameters with sampling variance < ", min_sample_var_pct, " * prior variance. One can change this threshold by setting the imabc.min_sample_var_pct option."))
      ...
      if (!is.null(output_directory) && validate_run) {
        ...
        interim_output_directory <- file.path(output_directory, interim_output_directory_sub_dir)

        if (!dir.exists(interim_output_directory)){
          dir.create(interim_output_directory)
        }
      ...
      # Stop algorithm if number of failed attempts to improve algorithm have hit the max desired
