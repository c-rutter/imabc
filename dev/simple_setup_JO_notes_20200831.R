...
## JO NOTE: We should consider simplifying this, as this appears to include the
## grouped targets as the top level concept. I would suggest that grouped
## targets would be at the same conceptual level as simple targets, and avoid
## the naming schemes like G2T1 when there's only one target.
...
# JO NOTE 1: we had talked about the following signature on 8/4 (this was also in imabc/dev/JO_notes_and_test_20200804.R)
# target_fun <- function(x, lower_bounds, upper_bounds, res, fun_seed) {

# JO NOTE 2: we could also consider a simple signature like function(x), where x
# is a list with elements such as lower_bounds, uper_bounds, res, and fun_seed,
# accessible via x$lower_bounds, etc.

target_fun <- function(x, lower_bounds, upper_bounds) {

  # JO NOTE: this would become unnecessary
  res <- c()

  # JO NOTE: This is the name use scheme we discussed
  # x$x1
  # x$x2
  # lower_bounds$t1
  # lower_bounds$t2
  # lower_bounds$corley.first
  ...
  # JO NOTE: we discussed consider adding results to named targets in res vector
  # res$t1 = 2
...
# JO NOTE: For all of the previous results matters, we should be able to point
# to a directory of a previous run only and automatically grab the relevant
# files such as:
#   - RunMetadata_*
#   - Good_SimulatedParameters_*
#   - etc.
# NB: If there are multiple time stamped runs in a directory (which is not
# usually how we've used imabc, but could be how people choose to use it), then
# the latest one would be chosen.
...
# JO NOTES: got the error:
# > new_priors <- define_priors(previous_run_priors = prev_run_meta)
# Error in define_priors(previous_run_priors = prev_run_meta) :
#   object 'imabc_minmax' not found



previous_results <- list(
  prev_run_meta = prev_run_meta,
  good_parm_draws = read.csv(paste(output_directory, "Good_SimulatedParameters_20200831_1342CDT.csv", sep = "/")),
  good_sim_parm = read.csv(paste(output_directory, "Good_SimulatedTargets_20200831_1342CDT.csv", sep = "/")),
  good_target_dist = read.csv(paste(output_directory, "Good_SimulatedDistances_20200831_1342CDT.csv", sep = "/")),
  mean_cov = read.csv(paste(output_directory, "MeanCovariance_20200831_1342CDT.csv", sep = "/"))
)
new_results <- imabc(
