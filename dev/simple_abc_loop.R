...
# target_list <- list(
#   t1 = list(
#     target_df #: target goal, upper bound, lower bound
#   ),
#   t2 = list(
#     target_df,
#     target_fn
...
fn1 <- function(x){x[1] + x[2] + rnorm(1,0,0.1)}
fn2 <- function(x){x[1] * x[2] + rnorm(1,0,0.1)}
...
dist_func <- function(x, targets) { # optional thresholds
  ...
  res[1] <- get.p.dist(fn1(x), targets$t1$target)
  res[2] <- get.p.dist(fn2(x), targets$t2$target)
...
parm.priors <- matrix(prior1, ncol=3) %>% as.data.frame() %>% mutate(parm = "parm1")

row <- matrix(prior2, ncol=3) %>% as.data.frame() %>% mutate(parm = "parm2")

parm.priors <- rbind(parm.priors, row)



# set column order
parm.priors.df <- parm.priors %>% rename(dist.type=V1, a=V2, b=V3) %>% select(parm, dist.type, a, b) %>%
  mutate(a = as.numeric(a), b = as.numeric(b))

# get parm names
parm.names <- parm.priors.df[,"parm"]
n.parms=length(parm.names)

# parm.priors <- add_prior(prior1)
# parm.priors <- add_prior(prior2)
  ...
  if(!all(unlist(parm.priors.df$parm) %in% parm.names)){
    print(paste0("Error: prior given for unspecified model parameter in given priors"))
    break
  }

  # calib.parm.names <- unlist(parm.priors.df$parm[parm.priors.df$sd>0])
  calib.parm.names <- unlist(parm.priors.df$parm)

  n.calib.parms <- length(calib.parm.names)
  if(N.cov.points==0) N.cov.points=25*n.calib.parms


  x=data.table(parm.priors.df %>% select(parm))
  names(x)="parm"
...
target1 = c(1.5, 1000, 1, 0,  0.001)
target2 = c(0.5, 1000, 1, 0.00001, 0.001)


target.specs <- matrix(target1, ncol=5) %>% as.data.frame() %>% mutate(data = "parm1")
row <- matrix(target2, ncol=5) %>% as.data.frame() %>% mutate(data = "parm2")
target.specs <- rbind(target.specs, row)

names(target.specs)=c("pct.male","m","n.ret","alpha","alpha.target", "data")
# set column order
target.specs <- target.specs  %>% select(data, m, pct.male, n.ret, alpha, alpha.target)
...
### Simplistic calib targets adapted from get.targets.R
  calib.targets <- vector("list", nrow(target.specs)+1)
  Range.p10mmIn10yrs=as.data.frame(matrix(c(0.00001, 0.5), ncol=2))
  names(Range.p10mmIn10yrs)=c('min','max')
  calib.targets[[1]]=list(Range.p10mmIn10yrs)

  Y.parm1=as.data.frame(matrix(c(12,1450), ncol=2))
  names(Y.parm1) = c("n.cancer", "n")
  Y.parm1$binomial=TRUE
  Y.parm1$poisson=FALSE

  Y = mutate(Y.parm1, p = n.cancer / n)
  Y$n.cancer=NULL
  setcolorder(Y,c('p','n','binomial','poisson'))
  Y = get.bounds(x=Y,alpha.level=target.specs[1,'alpha'])

  calib.targets[[2]] = Y

  Y.parm2=as.data.frame(matrix(c(7,1950), ncol=2))
  names(Y.parm2) = c("n.cancer", "n")
  Y.parm2$binomial=TRUE
  Y.parm2$poisson=FALSE

  Y = mutate(Y.parm2, p = n.cancer / n)
  Y$n.cancer=NULL
  setcolorder(Y,c('p','n','binomial','poisson'))
  Y = get.bounds(x=Y,alpha.level=target.specs[2,'alpha'])


  calib.targets[[3]] = Y

  names(calib.targets) = c("Range.p10mmIn10yrs","parm1", "parm2")
...
#####################################
  ...
  sim.parm.names=rep("",sum(target.specs[!bias.info,]$n.ret ))
  sim.parm.names[1] <- "parm1"
  sim.parm.names[2] <- "parm2"
...
#####################################
  ...
  source("R/init.draws.R")
  good.parm.draws <- init.draws(N = N.store, c(parm.names,"scaled.dist","sample.wt"))
  source("R/init.sims.R")
...
parm.draws[draws,calib.parm.names[1] :=   u.draws[,1]*(parm.priors.df[1,'b'] - parm.priors.df[1,'a']) + parm.priors.df[1,'a']]
  ...
  for(iter in start.iter:end.iter){
    ...
    combine_results <- function(LL1, LL2) {
      ll <- rbind(LL1$ll, LL2$ll)
      sp <- rbind(LL1$sp, LL2$sp)

      return(list(ll = as.data.frame(ll), sp = as.data.frame(sp)))
    }
    res <- foreach(i2 = 1:nrow(parms.to.run), .combine = combine_results) %dopar% {
      inp <- as.numeric(parms.to.run[i2, c("parm1", "parm2"), drop = TRUE])
    ...
    # --------------------------------------------------------------------------
    # CM NOTE: N.in>=N.post is likely to be always FALSE on first iteration
    if(((N.in>=N.post | !any(target.specs$update.alpha)) |
       (iter>=end.iter & N.in>0)) & iter != 1){
