...
# JO NOTE: this is the signature we talked about on 8/4, commented out to run the NA producing test
...
targets <- define_targets(
  # JO NOTE: single targets would potentially only have one name, based on 8/4 discussions
  t1 = add_target(
    target = 1.5,
    starting_range = c(1.0, 2.0),
    stopping_range = c(1.45, 1.55)
  )
  ,
  t2 = add_target(
    target = 0.5,
    starting_range = c(0.2, 0.9),
    stopping_range = c(0.45, 0.55)
  )
  # JO NOTE: how the naming of targets could work, based on 8/4 discussions
  # ,
  # corley = group_targets(
  #   first = add_target(
  #     target = 0.5,
  #     starting_range = c(0.2, 0.9),
  #     stopping_range = c(0.45, 0.55)
...
priors <- define_priors(
  ...
  x2 = add_prior(
    dist_base_name = "norm",
    mean = 1, sd = 2, # Inputs into *truncnorm
# JO NOTE: if min and max are not provided, consider setting them as -Inf, Inf or equivalent
    min = -100, max = 100 # Info required for imabc not defined in *truncnorm
  )
)
